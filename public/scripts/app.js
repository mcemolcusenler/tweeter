...
var createTextElement = function (twitObject) {
  ...
  var $image = $(`<img src="${$userAvatar}">`);
  var $userName = twitObject.user.name;
  var $pUserName = $(`<p class="user-name">${$userName}</p>`);
  var $userHandle = twitObject.user.handle;
  var $pUserHandle = $(`<p class="user-handle">${$userHandle}</p>`);
  var $userContent = twitObject.content.text;
  var $pContent = $(`<p>${$userContent}</p>`);
  var $footer = $('<footer>');
  var $createdAt = twitObject.created_at;
  var $pCreatedAt = $(`<p>${$createdAt}</p>`);
...
var data = [
  ...
  $sectionAllTweets.append($storeTweet);
});